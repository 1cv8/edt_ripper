...
Функция ИгнорироватьПравилоПоКоду(ID) Экспорт
	...
	//Если ИсключенияПоКоду = Неопределено Тогда
	//	Возврат Ложь;
	//КонецЕсли
...
Процедура ПриСозданииОбъекта()
	...
	Лог.Информация("Чтение настроек проверки");
	...
	Если СчФильтровID > 0 ИЛИ СчФильтровКод > 0 Тогда
		...
		Лог.Информация("Подключены фильтры:");
		Если СчФильтровID > 0 Тогда
			Лог.Информация("   по ID: %1", СчФильтровID);
		КонецЕсли;
		Если СчФильтровКод > 0 Тогда
			Лог.Информация("   по коду: %1", СчФильтровКод);
		КонецЕсли;
